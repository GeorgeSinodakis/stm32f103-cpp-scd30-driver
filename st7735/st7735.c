...
void st7735_Draw_Font_Pixel(uint8_t x_pos, uint8_t y_pos, uint16_t colour, uint8_t pixel_size)
{
	uint8_t i = 0x00;
...
void st7735_print_char(uint8_t x_pos, uint8_t y_pos, uint8_t font_size, uint16_t colour, uint16_t back_colour, char ch)
{
...
void st7735_print_str(uint8_t x_pos, uint8_t y_pos, uint8_t font_size, uint16_t colour, uint16_t back_colour, std::string str)
{
	for (auto &c : str)
	{
		if (c != '\0')
...
void st7735_line(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint16_t color)
{
	...
	for (; x0 <= x1; x0++)
	{
		if (steep)
		{
			st7735_rect(y0, x0, y0, x0, color);
		}
		else
		{
			st7735_rect(x0, y0, x0, y0, color);
		}
		err -= dy;
		if (err < 0)
		{
...
void swap(uint8_t *value1, uint8_t *value2)
{
	uint8_t buffer = *value1;
	*value1 = *value2;
	*value2 = buffer;
}